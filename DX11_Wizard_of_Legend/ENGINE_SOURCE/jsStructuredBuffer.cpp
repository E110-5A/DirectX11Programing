...
namespace js::graphics
{
	...
	bool StructuredBuffer::Create(eSRVType type, UINT size, UINT stride, void* data, bool cpuAccess)
	{
		mType = type;
		mSize = size;
		mStride = stride;
	...
	void StructuredBuffer::BindUAV(eShaderStage stage, UINT slot)
	{
		mUAVSlot = slot;
		UINT count = -1;
		GetDevice()->BindUnorderedAccessView(slot, 1, mUAV.GetAddressOf(), &count);
	}
	void StructuredBuffer::Clear()
	{
		...
		UINT count = -1;
		GetDevice()->BindUnorderedAccessView(mUAVSlot, 1, &uav, &count);
	}
	...
	bool StructuredBuffer::createRWBuffer()
	{
		...
		wDesc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
		...
		rDesc.CPUAccessFlags = D3D10_CPU_ACCESS_READ;
